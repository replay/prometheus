...
func TestSetCompactionFailed(t *testing.T) {
	...
	b, err := OpenBlock(nil, blockDir, nil, nil, nil)
	require.NoError(t, err)
...
func TestCreateBlock(t *testing.T) {
	tmpdir := t.TempDir()
	b, err := OpenBlock(nil, createBlock(t, tmpdir, genSeries(1, 1, 0, 10)), nil, nil, nil)
	require.NoError(t, err)
	require.NoError(t, b.Close())
}
...
func TestBlockSize(t *testing.T) {
	...
	// Create a block and compare the reported size vs actual disk size.
	{
		...
		blockInit, err = OpenBlock(nil, blockDirInit, nil, nil, nil)
		require.NoError(t, err)
		defer func() {
	...
	// Delete some series and check the sizes again.
	{
		...
		blockAfterCompact, err := OpenBlock(nil, filepath.Join(tmpdir, blockDirsAfterCompact[0].String()), nil, nil, nil)
		require.NoError(t, err)
		defer func() {
